...
export default function App() {
  ...
  const [shareModal, setShareModal] = useState<{ open: boolean, mode: 'export' | 'import', data?: string }>({ open: false, mode: 'export' });
  ...
  const handleExportStaff = (id: string) => {
    const data = localStorage.getItem(DATA_PREFIX + id);
    if (data) {
      const bundle = {
        type: 'SINGLE_STAFF_EXPORT',
        data: JSON.parse(data)
      };
      const code = btoa(unescape(encodeURIComponent(JSON.stringify(bundle))));
      setShareModal({ open: true, mode: 'export', data: code });
    }
  };

  const handleImportStaff = (code: string) => {
    try {
      const json = decodeURIComponent(escape(atob(code)));
      const bundle = JSON.parse(json);
      if (bundle.type !== 'SINGLE_STAFF_EXPORT') throw new Error('Invalid type');

      const newId = generateId();
      const importedData = bundle.data;
      
      // Ensure we have a fresh ID for the imported data to avoid conflicts
      importedData.metadata.id = newId;
      importedData.metadata.updatedAt = Date.now();
      
      const newSummary = {
        id: newId,
        name: importedData.metadata.name,
        store: importedData.metadata.store,
        date: importedData.metadata.date,
        updatedAt: Date.now()
      };
      
      localStorage.setItem(DATA_PREFIX + newId, JSON.stringify(importedData));
      setStaffList(prev => {
        const newList = [newSummary, ...prev];
        localStorage.setItem(STAFF_INDEX_KEY, JSON.stringify(newList));
        return newList;
      });
      setShareModal({ open: false, mode: 'export' });
      alert(`${importedData.metadata.name}さんのデータを新規スタッフとして読み込みました。`);
    } catch (e) {
      alert('コードの読み込みに失敗しました。無効なコードである可能性があります。');
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
        ...
        <div className="space-y-4 text-center">
            <h1 className="text-white text-3xl font-black tracking-tight">QB Evaluation System</h1>
            <p className="text-slate-400 font-medium tracking-widest uppercase text-sm">Loading Staff Data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <>
        <MenuPage
          ...
          onExport={() => handleExportStaff(selectedStaffSummary!.id)}
        ...
        <ShareModal 
          ...
          mode={shareModal.mode} 
          data={shareModal.data} 
          staffName={selectedStaffSummary?.name}
          onClose={() => setShareModal({ ...shareModal, open: false })}
        />
      </>
    );
              ...
              return (
                <div key={idx} className="page-break">
                  ...
                  <div className="mt-4 mb-2">
                    ...
                    <div className="print-columns-3 text-[7pt]">
